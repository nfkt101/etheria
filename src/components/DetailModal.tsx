...
export default function DetailModal() {
  ...
  const { downloads, triggerDownload } = useDownloadsStore();

  if (!item) return null;

  const downloadStatus = downloads[item.id];
    ...
    <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-black/85 backdrop-blur-xl animate-entrance-hero overflow-y-auto">
      ...
      <div className="relative w-full max-w-4xl bg-[#141416]/90 border border-white/10 rounded-2xl md:rounded-3xl shadow-[0_0_50px_rgba(205,189,255,0.15)] overflow-hidden z-10 max-h-[90vh] overflow-y-auto hide-scrollbar">
        ...
        <div className="relative w-full h-[320px] md:h-[420px] overflow-hidden">
          <img src={item.image} alt={item.title} className="w-full h-full object-cover transform scale-102 transition-transform duration-700" />
          ...
          <div className="absolute bottom-6 left-0 w-full px-6 md:px-8">
            <div className="flex flex-wrap gap-2 items-center mb-2">
              <span className="px-2.5 py-0.5 bg-primary/20 border border-primary/40 rounded text-primary text-[10px] md:text-xs font-bold uppercase tracking-wider">
                {item.type === 'movie' ? 'Cinematic Film' : 'Exclusive TV Series'}
              </span>
        ...
        <div className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-3 gap-8 text-on-background select-text">
          <div className="md:col-span-2 space-y-6">
            <div className="flex flex-wrap gap-3">
              ...
              >
                {isFavorite(item.id) ? <Check className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
                My List
              </button>
              <button
              ...
              <div className="space-y-4 pt-2 border-t border-white/5">
                ...
                <div className="space-y-3">
                  {item.episodes.map((ep) => (
                    <div
                      key={ep.id}
                      onClick={() => openPlayer({ ...item, title: `${item.title}: ${ep.title}`, tagline: ep.description })}
            ...
            {item.tags.length > 0 && (
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {item.tags.map((tag) => (
                    <span key={tag} className="px-2.5 py-1 text-xs font-semibold rounded bg-neutral-900 border border-white/15 text-[#cac3d8]">
                      {tag}
                    </span>
                  ))}
                </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-white font-medium">{item.releaseYear}</div>
                </div>
              </div>
              <div className="flex items-center gap-3">
            ...
            {item.genres.length > 0 && (
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {item.genres.map((g) => (
                    <span key={g} className="px-2.5 py-0.5 rounded-full text-xs bg-white/5 border border-white/10 text-on-surface-variant">
                      {g}
                    </span>
                  ))}
                </div>
